...
rnd_list = list([randint(0, 101) for i in range(k+1)]) # задаем случайный список
if rnd_list[0] == 0: # если будет равен 0, томногочлен может быть неверным
    rnd_list[0] = randint(1, 101)
print("Случайный список: ", rnd_list)

def get_polynomial(k, rnd_list): 
    str1 = ['*x**']*(k-1) + ['*x']
    polynomial = [[a, b, c] for a, b, c  in itertools.zip_longest(rnd_list, str1, range(k, 1, -1), fillvalue = '') if a !=0]
    ...
    print(polynomial)
    polynomial[-1] = ' = 0' # меняем последний '+' на '= 0'
    return "".join(map(str, polynomial)).replace(' 1*x',' x') # возвращаем строку

list = get_polynomial(k, rnd_list)
print(list)
